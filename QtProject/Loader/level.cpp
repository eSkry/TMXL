...
void Level::loadTiles(pugi::xml_document &doc){
    pugi::xml_node map        = doc.child("map");
    ...
    while (tileset){
        ...
        while (tile){
            ...
            vector<LPropertyData*> tempPropsVec;

            while (property){
                LPropertyData *tempProp = new LPropertyData;

                tempProp->sName = property.attribute("name").as_string();
                tempProp->pValue = static_cast<string>(property.attribute("value").as_string());
            ...
            tempTileSet->mPropertys.insert( pair<int, vector<LPropertyData*>>(iIDTile, tempPropsVec) );
...
void Level::loadTileData(pugi::xml_document &doc){
    pugi::xml_node map        = doc.child("map");
    pugi::xml_node properties;
    pugi::xml_node property;
    ...
    while (layer){
        ...
        tempLayer->sName    = layer.attribute("name").as_string();
        ...
        // Параметры в слое
        properties = layer.child("properties");
        while (properties){
            properties = layer.child("properties");
            property = properties.child("property");

            while (property){
                LPropertyData* tempProps = new LPropertyData;

                tempProps->sName = property.attribute("name").as_string();
                pugi::xml_attribute type = property.attribute("type");

                if (type != NULL){
                    tempProps->sType = type.as_string();
                }

                tempProps->pValue = property.attribute("value").as_string();

                tempLayer->vPropertys.push_back(tempProps);
                property = property.next_sibling("property");
            }
...
LPropertyData* Level::getTileConcrProperty(string tilesetName, int tileID, string propertyName){
    vector<LPropertyData*> temp = getTilePropertys(tilesetName, tileID);
    LPropertyData* prop = nullptr;
...
vector<LPropertyData*> Level::getTilePropertys(string tilesetName, int tileID){
    ...
    vector<LPropertyData*> clearVector;
    return clearVector;
}

vector<LPropertyData*> Level::getLayerPropertys(string layerName){
    vector<LPropertyData*> temp;
    for (auto it = vLayers.begin(); it != vLayers.end(); it++){
        if (strcmp((*it)->sName.c_str(), layerName.c_str()) == 0){
            temp = (*it)->vPropertys;
        }
    }
...
LPropertyData* Level::getLayerConctProperty(string layerName, string propertyName){
    vector<LPropertyData*> temp = getLayerPropertys(layerName);
    for (auto it = temp.begin(); it != temp.end(); it++){
        if (strcmp( (*it)->sName.c_str(), propertyName.c_str() ) == 0){
            return (*it);
        }
